...
module.exports = function (params, incrementType) {
	try {
		...
		for (let i in params.paths) {
			...
			if (content) {
				if (obj.match.test(content)) {
					...
					console.log(`Not found => ${obj.match} in file => ${pathFile}`);
				}
			}
		}
	} catch (e) {
...
function checkIncrementType(incrementType) {
	if (! incrementType || ! incrementType.match(/^(ooo|oooa|ooi|ooia|oio|ioo)$/)) {
		let newIncrementType = '---';
